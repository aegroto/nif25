...
def load_device(force_cpu=False):
    if force_cpu:
        ...
        return torch.device(os.environ.get("NIF_DEVICE") or "cuda")

def set_reproducibility():
    seed = os.environ.get("NIF_SEED")
    if seed:
        torch.random.manual_seed(seed)
    else:
        # TODO: Remove this and force reproducibility via seed
        seed = 1337
...
def get_or(value, alternative):
    if value is None:
        return alternative
    else:
        return value

def dict_get_or(table, key, alternative):
    try:
        return table[key]
    except KeyError:
        return alternative

def clearlines(n, width=100):
    return
    for _ in range(n):
...
def mem_usage():
    return torch.cuda.memory_reserved(1) / (1024 * 1024 * 1024)
